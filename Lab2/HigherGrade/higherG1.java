...
//Implement insertion sort and Prints in descending order.
...
public class higherG1 {
	...
	//return true if the element a is smaller than the element b. If not, it is false.
	 ...
	 //return true if the array is sorted in descending order, false if not.
	...
	//change the highest index with lowest index and then increments i and decreases last
				int last = size -1;      
				for(int i = 0; i < last; i++) {
					int temp = a[i];       // temp value requires O(1) extra memory
					a[i] = a[last];
					a[last] = temp;
					last-- ;
				}
